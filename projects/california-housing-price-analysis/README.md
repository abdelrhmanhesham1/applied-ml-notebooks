...
`model.fit()` takes one line and tells you nothing about what actually happens inside it. Most of my other regression work in this collection calls a library optimizer and trusts it — a good R² without being able to explain, on a whiteboard, why the loss goes down the way it does. That started to bother me, so this is the notebook where I stopped calling the library function and wrote batch, stochastic, mini-batch, and momentum gradient descent myself, on the same dataset, so I could watch their convergence curves diverge from each other and confirm my intuition about *why* momentum stabilizes and stochastic doesn't matched what the numbers actually did.
